...
class SOM(object):
    ...
    def __init__(self, name, Data, mapsize=None, norm_method='var', initmethod='pca', neigh='Gaussian'):
        """
        name and data
        neigh: ['Bubble', 'Gaussian']
        """
        self.name = name
        self.data_raw = Data
        if norm_method == 'var':
    ...
    def set_topology(self, mapsize=None, mapshape='planar', lattice='rect', mask=None, compname=None):
        ...
        if mapsize == None:
            ...
            if len(mapsize)==2:
                ...
                s =  int (mapsize[0]/2)
                #TODO: there is a pb here: should be
                # self.mapsize = [s, s] ?
    ...
    #set attributes of the initialization algorithm
    ...
    def init_map(self):
        if  getattr(self, 'initmethod') == 'random':
            #produces random values in the range of [min,max] of each dimension based on a uniform distribution
            dim = getattr(self, 'dim')
            nnodes = getattr(self, 'nnodes')
            data = getattr(self,'data')
            mn = np.tile(np.min(data, axis=0), (nnodes, 1))
            mx = np.tile(np.max(data, axis=0), (nnodes, 1))
            setattr(self, 'codebook', mn + (mx-mn)*(np.random.rand(nnodes, dim)))
        elif getattr(self, 'initmethod') == 'pca':
            #based on two largest eigenvalues of correlation matrix
    ...
    def train(self, trainlen=None, n_job=1, shared_memory='no', verbose='on'):
        ...
        if verbose == 'on' or verbose == 'final':
            ts = round(time() - t0, 3)
            print
    ...
    def project_data(self, data, n_method="var"):
        ...
        data = normalize_by(data_raw, data, n_method=n_method)
    ...
    def cluster(self,method='Kmeans',n_clusters=8, n_method='var'):
        import sklearn.cluster as clust
        km= clust.KMeans(n_clusters=n_clusters)
        labels = km.fit_predict(denormalize_by(self.data_raw, self.codebook, n_method =n_method))
    ...
    def hit_map_cluster_number(self,data=None, n_method="var"):
        if hasattr(self, 'cluster_labels'):
            codebook = getattr(self, 'cluster_labels')
        else:
            print 'clustering based on default parameters...'
            codebook = self.cluster(n_method=n_method)
        msz =  getattr(self, 'mapsize')
        ...
        if data == None:
            data_tr = getattr(self, 'data_raw')
            proj = self.project_data(data_tr, n_method=n_method)
            coord = self.ind_to_xy(proj)
...
def batchtrain(self, njob=1, phase=None, shared_memory='no', verbose='on'):
    ...
    Since it is a fixed value we can skip it during bmu finding for each data point, but later we need it to calculate quantification error
    """
    ...
    for i in range(trainlen):
        if neigh_func == 'Gaussian':
            #in case of Gaussian neighborhood
...
def rect_dist(self,bmu_ind):
    ...
    #bmu_ind should be an integer between 0 to nnodes-1
    if 0 <= bmu_ind <= (rows * cols -1):
...
def normalize_by(data_raw, data, n_method='var'):
    #methods  = ['var','range','log','logistic','histD','histC']
    #status = ['done', 'undone']
...
def denormalize_by(data_by, n_vect, n_method = 'var'):
    ...
    if n_method == 'var':
        ...
        scaler = preprocessing.MaxAbsScaler()
        scaler.fit(data_by)
        vect = scaler.inverse_transform(n_vect)
        return n_vect
    else:
